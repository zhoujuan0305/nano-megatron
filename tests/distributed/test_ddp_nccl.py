...
torchrun --standalone --nproc_per_node=2 --master_addr=127.0.0.1 \
  -m pytest tests/distributed/test_ddp_nccl.py -v -s --import-mode=importlib
"""
...
def _assert_tp_grads_match_ref(tp, ref, cfg, rank: int, tp_sz: int) -> None:
    for name, tp_p in tp.named_parameters():
        ...
        if "qkv_proj" in name and cfg.use_fused_qkv:
            ...
            assert torch.allclose(tp_g, expected, atol=_ATOL, rtol=_RTOL), (
            ...
            assert torch.allclose(tp_g, ref_g, atol=_ATOL, rtol=_RTOL), (
                f"grad mismatch on {name}"
            )
...
def _run_torchrun(nproc: int, test_id: str) -> None:
    ...
    cmd = [
        ...
        "-m",
        "pytest",
        f"tests/distributed/test_ddp_nccl.py::{test_id}",
        "-v",
        "-s",
        "--import-mode=importlib",
    ]
    env = os.environ.copy()
    env["NANO_MEGATRON_DDP_NCCL_WORKER"] = "1"
...
@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_DDP_NCCL_WORKER") == "1", reason="launcher only"
)
def test_launch_dp2_grad_matches_reference_nccl():
    _run_torchrun(2, "test_worker_dp2_grad_matches_reference_nccl")


@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_DDP_NCCL_WORKER") != "1", reason="worker only"
)
def test_worker_dp2_grad_matches_reference_nccl():
    ...
    ctx = initialize_parallel(
        ParallelConfig(data_parallel_size=2), dist_backend="nccl"
    )
    ...
    x_global = torch.randn(4, 8, device=device)
    ...
    for (name, p_ddp), p_ref in zip(
        ...
        assert torch.allclose(
            p_ddp.grad, p_ref.grad, atol=_ATOL, rtol=_RTOL
        ), f"grad mismatch on {name}"
...
def test_launch_tp2_dp2_grad_matches_reference_nccl():
    _run_torchrun(4, "test_worker_tp2_dp2_grad_matches_reference_nccl")


@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_DDP_NCCL_WORKER") != "1", reason="worker only"
)
def test_worker_tp2_dp2_grad_matches_reference_nccl():
    from nano_megatron.distributed import DistributedDataParallel
    ...
    tp = build_tp_gpt_from_reference(ref, ctx).to(device)
    ...
    torch.manual_seed(1)
    seq = 8
    ids_global = torch.randint(0, cfg.vocab_size, (4, seq), device=device)
    ...
    # Reload after DDP init broadcast so ref matches the synchronized TP shards.
    # build_tp_gpt_from_reference already shards from seed-0 ref; DDP broadcasts
    # params within each DP group from DP rank 0 (same TP rank), so weights stay
    # consistent with the original ref for the TP shard layout.
