...
torchrun --standalone --nproc_per_node=2 --master_addr=127.0.0.1 \
  -m pytest tests/distributed/test_sp_gloo.py -v -s --import-mode=importlib
"""
...
def _run_torchrun(nproc: int, test_id: str) -> None:
    ...
    cmd = [
        ...
        "-m",
        "pytest",
        f"tests/distributed/test_sp_gloo.py::{test_id}",
        "-v",
        "-s",
        "--import-mode=importlib",
    ]
    env = os.environ.copy()
    env["NANO_MEGATRON_SP_WORKER"] = "1"
...
@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_SP_WORKER") == "1", reason="launcher only"
)
def test_launch_sp_vs_tp_forward_backward_gloo():
    _run_torchrun(2, "test_worker_sp_vs_tp_forward_backward_gloo")


@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_SP_WORKER") != "1", reason="worker only"
)
def test_worker_sp_vs_tp_forward_backward_gloo():
    """SP=True TPGPT must match SP=False TPGPT on local vocab logits and grads.
...
def test_launch_sp_vs_tp_fused_qkv_forward_backward_gloo():
    _run_torchrun(2, "test_worker_sp_vs_tp_fused_qkv_forward_backward_gloo")


@pytest.mark.skipif(
    os.environ.get("NANO_MEGATRON_SP_WORKER") != "1", reason="worker only"
)
def test_worker_sp_vs_tp_fused_qkv_forward_backward_gloo():
    """Same SP vs non-SP check with fused QKV path."""
    from nano_megatron.model import build_tp_gpt_from_reference
    from nano_megatron.parallel import (
        ParallelConfig,
        destroy_parallel,
