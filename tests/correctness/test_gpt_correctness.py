...
class TestGPTCorrectness:
    def test_forward_shape_various_sizes(self) -> None:
        configs = [
            GPTConfig(
                hidden_size=64, num_layers=2, num_attention_heads=4, seq_length=16, vocab_size=512
            ),
            GPTConfig(
                hidden_size=128, num_layers=4, num_attention_heads=8, seq_length=32, vocab_size=1024
            ),
            GPTConfig(
                hidden_size=256, num_layers=2, num_attention_heads=8, seq_length=64, vocab_size=2048
            ),
        ]
        for config in configs:
            ...
            assert logits.shape == (B, config.seq_length, config.vocab_size)
    ...
    def test_backward_gradient_numerics(self) -> None:
        set_seed(42)
        config = GPTConfig(hidden_size=128, num_layers=2, num_attention_heads=8)
        model = GPTModel(config)
        input_ids = torch.randint(0, config.vocab_size, (2, config.seq_length))
        labels = input_ids.clone()
        _, loss = model(input_ids, labels=labels)
        ...
        assert total_norm > 0, "Total gradient norm is zero"
        assert not torch.isnan(torch.tensor(total_norm)), "Gradient norm is NaN"
...
class TestCausalMaskCorrectness:
    def test_prefix_consistency(self) -> None:
        config = GPTConfig(
            hidden_size=64, num_layers=2, num_attention_heads=4, seq_length=32, vocab_size=128
        )
        model = GPTModel(config)
        model.eval()
